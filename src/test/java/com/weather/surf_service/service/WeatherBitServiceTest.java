...
//TODO I am not using below annotation because it is junit test, not integration
//@SpringBootTest
class WeatherBitServiceTest {
    ...
    public void dateMoreThan16DaysFromNowShouldThrowException() {
        ...
        //when
        Exception exception1 = assertThrows(DateToFarAwayException.class, () -> weatherBitService.calculateDateRangeFromToday(dateString1));
        Exception exception2 = assertThrows(DateToFarAwayException.class, () -> weatherBitService.calculateDateRangeFromToday(dateString2));
        Exception exception3 = assertThrows(DateToFarAwayException.class, () -> weatherBitService.calculateDateRangeFromToday(dateString3));
        String expectedMessage = "Provided date is out of range.";
        List<String> logEvents = logCaptor.getErrorLogs();
        //then
        assertThat(exception1).isInstanceOf(DateToFarAwayException.class).hasMessageContaining(expectedMessage);
        assertThat(exception2).isInstanceOf(DateToFarAwayException.class).hasMessageContaining(expectedMessage);
        assertThat(exception3).isInstanceOf(DateToFarAwayException.class).hasMessageContaining(expectedMessage);
        assertThat(logEvents).hasSize(3);
    }
